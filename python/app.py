...
def show_project(project_btn, ok_project_description_btn, add_test_service_btn, remove_test_service_btn, cancel_project_description_btn, project_table_cellValueChanged, type_input, client_input, project_description_style, project_list_table_rowData, project_table_rowData, start_date_input, stop_date_input,  project_input, project_id_input, location_input, project_manager_input, category_input, PL_input, contact_input, quote_input, quote2_input, PO_input, draft_input, report_input, time_input, invoice_input, status_input, loading_screen_children, loading_screen_style, project_number):
    ...
    if triggered_id == 'project_list_table' and project_description_style['display'] == 'none':
        ...
        try:
            ...
            connection.ping(reconnect=True)
            cursor.execute(sql, (start_date_input, stop_date_input, location_input, project_input, project_id_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, quote_input, quote2_input, PO_input, draft_input, report_input, time_input, invoice_input, status_input, project_number))
            connection.commit()
...
def add_project(ok_btn, add_project, project_list_table_rowData, project_list_table_selectedRows, loading_screen_style, project_input, project_id_input, location_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, quote_input, Quote2, PO_input, draft_input, report_input, time_input, invoice_input, status_input, start_date_input, stop_date_input):
    if ok_btn!= 0:
        try:
            ...
            if add_project == 1:
                ...
                sql = ("INSERT INTO project_tracker (`Start Date`, `End Date`, Location, Number, Project, `Project ID`, PM, Category, Typ, Client, PL, Contact, Quote, Quote2, PO, DRAFT, Report, `Work [hours]`, Invoice, Status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)")
                connection.ping(reconnect=True)
                cursor.execute(sql, (str(start_date_input), str(stop_date_input), location_input, number, project_input, project_id_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, None, None, None, None, None, 0, None, status_input))
                connection.commit()
