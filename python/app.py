...
project_manager_input = html.Div(dbc.Stack([html.Label('Project manager', style={'fontWeight':'bold'}), dcc.Dropdown(['JS','PS','RG','LK','CL', 'AJ'],id="project_manager_input",style={'width':150, "height": 35})]))
...
PL_input = html.Div(dbc.Stack([html.Label('PL', style={'fontWeight':'bold'}), dcc.Dropdown(['AC/DC - Lighting', 'ADC', 'Analog', 'Automotive', 'Other', 'Power Magnetics', 'Sensor', 'STD DC/DC','Telecom + Computing','Automotive BMS','Axign'],id="PL_input",style={'width':200, "height": 35})]))
...
project_window = html.Div([
                                ...
                                id='project_window',style={'height':400,'width':1000,'display':'none','position':'fixed','top':'40%','left':'35%','backgroundColor':"#e0e0e0",'padding':'10px 10px','boxShadow':'0px 4px 8px rgba(0,0,0,0.1)','zIndex':'2002','borderRadius':'8px','overflow':'auto'})
...
project_manager_input = html.Div(dbc.Stack([html.Label('Project manager', style={'fontWeight':'bold'}), dcc.Dropdown(['JS','PS','RG','LK','CL', 'AJ'],id="project_manager_description_input",style={'width':150})]))
...
PL_input = html.Div(dbc.Stack([html.Label('PL', style={'fontWeight':'bold'}), dcc.Dropdown(['AC/DC - Lighting', 'ADC', 'Analog', 'Automotive', 'Other', 'Power Magnetics', 'Sensor', 'STD DC/DC','Telecom + Computing','Automotive BMS','Axign'],id="PL_description_input",style={'width':200})]))
...
project_description = html.Div([
                    ...
                    dbc.Stack([project_input, project_ID, location_input, project_manager_input, start_date_input, stop_date_input], direction='horizontal',gap=3, style={'margin-bottom':10}),
                    dbc.Stack([category_input, type_input, client_input, PO_input, PL_input], direction='horizontal',gap=3, style={'margin-bottom':10}),
...
def show_project(project_btn, ok_project_description_btn, add_test_service_btn, remove_test_service_btn, cancel_project_description_btn, project_table_cellValueChanged, type_input, client_input, project_description_style, project_list_table_rowData, project_table_rowData, start_date_input, stop_date_input,  project_input, project_id_input, location_input, project_manager_input, category_input, PL_input, contact_input, quote_input, quote2_input, PO_input, draft_input, report_input, time_input, invoice_input, loading_screen_children, loading_screen_style, project_number):
    ...
    if triggered_id == 'project_list_table' and project_description_style['display'] == 'none':
        ...
        try:
            ...
            sql = "UPDATE project_tracker SET `Start Date` = %s, `End Date` = %s, Location = %s, Project = %s, Project ID = %s, PM = %s, Category = %s, Typ = %s, Client = %s, PL = %s, Contact = %s, Quote = %s, Quote2 = %s, PO = %s, DRAFT = %s, Report = %s, `Work [hours]` = %s, Invoice = %s WHERE Number = %s"
            connection.ping(reconnect=True)
            cursor.execute(sql, (start_date_input, stop_date_input, location_input, project_input, project_id_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, quote_input, quote2_input, PO_input, draft_input, report_input, time_input, invoice_input, project_number))
            connection.commit()
        ...
        raise PreventUpdate
    return project_description_style, project_description_label_children, project_list_table_rowData, project_table_rowData, start_date_input, stop_date_input, project_input, project_id_input, location_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, quote_input, quote2_input, PO_input, draft_input, report_input, time_input, invoice_input, project_number, loading_screen_children, loading_screen_style
...
def load_backup(contents, loading_screen_style):
    ...
    if trigger_id == 'upload-backup':
        try:
            ...
            sql = "DROP TABLE IF EXISTS project_tracker"
            cursor.execute(sql)
            sql = "DROP TABLE IF EXISTS project_description"
            cursor.execute(sql)
            ...
            sql = ("INSERT INTO project_tracker (`Start Date`, `End Date`, Location, Number, Project, `Project ID`, PM, Category, Typ, Client, PL, Contact, Quote, Quote2, PO, DRAFT, Report, `Work [hours]`, Invoice, Notes) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)")
...
def add_project(ok_btn, add_project, project_list_table_rowData, project_list_table_selectedRows, loading_screen_style, project_input, project_id_input, location_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, quote_input, Quote2, PO_input, draft_input, report_input, time_input, invoice_input, start_date_input, stop_date_input):
    if ok_btn!= 0:
        try:
            ...
            if add_project == 1:
                ...
                sql = ("INSERT INTO project_tracker (`Start Date`, `End Date`, Location, Number, Project, `Project ID`, PM, Category, Typ, Client, PL, Contact, Quote, Quote2, PO, DRAFT, Report, `Work [hours]`, Invoice, Notes) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)")
                connection.ping(reconnect=True)
                cursor.execute(sql, (str(start_date_input), str(stop_date_input), location_input, number, project_input, project_id_input, project_manager_input, category_input, type_input, client_input, PL_input, contact_input, None, None, None, None, None, 0, None, None))
                connection.commit()
